...
export function normalizeConfig(raw: RawConfig = {}) {
  const includePaths =
    Array.isArray(raw.includePaths) && raw.includePaths.length > 0 ? raw.includePaths : ['.'];
  const maxSizeKb =
    typeof raw.maxSizeKb === 'number' && Number.isInteger(raw.maxSizeKb) && raw.maxSizeKb > 0
      ? raw.maxSizeKb
      : 500;
  const allowedFormats =
    Array.isArray(raw.allowedFormats) && raw.allowedFormats.length > 0
      ? raw.allowedFormats
      : ['webp'];
  ...
  return {
    ...
    maxCompressionIterations: Number.isInteger(raw.maxCompressionIterations)
      ? raw.maxCompressionIterations
      : 8,
    resizeFallback: {
